...
// // one hundred more of these console.logs....
// console.log("Hello World");
...
Number.prototype.times = function (f) {
  let n = this.valueOf();
  if (n < 0) throw new RangeError('Cannot call method "times" on negative number.');
  if (Math.round(n) !== n) throw new RangeError('Cannot call method "times" on non-integer number.');
...
// Solution 5
Number.prototype.times = function (f) {
  for (var i = 0, cnt = this; i < cnt; f(i++)) ;
}


// Solution 6
Number.prototype.times = function (f) {
  Array.from({length: this}, (x, i) => f(i))
}
...
// Solution 12
Number.prototype.times = function (f) {
  Array.from({length: Number(this)}).forEach((_, i) => f(i))
}
...
Number.prototype.times = function (f) {
  var that=0;
  while (that<this) f(that++);
}


// Solution 14
Number.prototype.times=function(f,i=0){while(i<this)f(i++)}