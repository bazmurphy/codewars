...
// Solution 11
const cutCube=(v,n)=>!(Math.cbrt(v/n)%1)&&!(Math.cbrt(n)%1);


// Solution 12
cutCube = (volume,n) => !(Math.cbrt(n) % 1) && !(Math.cbrt(volume / n) % 1)


// Solution 13
function cutCube(volume,n){
  var originalSide = Math.cbrt(volume);
  var cutSide = Math.cbrt(volume/n);
  return cutSide === Math.round(cutSide) && originalSide % cutSide === 0;  
}


// Solution 14
cutCube=(v,n)=>[v/n,n].map(Math.cbrt).every(Number.isInteger)