...
// Solution 8
const splitAndMerge = (S, s) => S.split(' ').map(e => e.split('').join(s)).join(' ')
...
// Solution 10
const splitAndMerge = (string, separator) => string.replace(/\w(?=\S)/g, `$&` + separator);


// Solution 11
splitAndMerge = (s, b) => s.split(' ').map(e => e.split('').join(b)).join(' ');
...
// Solution 13
let splitAndMerge=(str,sp)=>str.split(" ").map(x=>x.split("").join(sp)).join(" ");
