...
function cutIt(arr){
  return arr.map((element) => element.slice(0, Math.min(...arr.map((element) => element.length))));
}
...
// Solution 7
cutIt = function (arr) {
  var min = arr.reduce(function(s, c) { return s > c.length || s == 0 ? c.length : s; }, 0)
...
// Solution 8
const cutIt = a => {
  const m = Math.min(...a.map(x=>x.length))
  return a.map(x=>x.substr(0,m))
}
...
function cutIt(arr){
  ...
  return arr.map(function(val){
    return val.slice(0,Math.min.apply(null,x));
  });
}


// Solution 14
const cutIt = (arr) => arr.map(el => el.slice(0, (Math.min.apply(0, arr.map(e => e.length)))))