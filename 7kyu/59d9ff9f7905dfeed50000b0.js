...
function solve(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let totalLettersInPosition = 0;

    for (let j = 0; j < arr[i].length; j++) {
      const letter = arr[i][j].toLowerCase();

      letterPositionInString = j + 1;

      alphabetPositionOfLetter = alphabet.indexOf(letter);

      if (letterPositionInString === alphabetPositionOfLetter) {
        totalLettersInPosition++;
      }
    }

    resultArray.push(totalLettersInPosition);
  }
